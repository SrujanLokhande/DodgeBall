...
void ADodgeBallCharacter::Throw_Implementation()
{
	if(!IsValid(BallToSpawn)) return;
	...
	const FVector spawnLocation = GetActorLocation() + GetControlRotation().Vector() * 20.0f;
	const TObjectPtr<UWorld> world = GetWorld();

	if (!IsValid(world)) return;

	world->SpawnActor<ABall>(BallToSpawn, spawnLocation, spawnRotation, shootParams);

	
}

float ADodgeBallCharacter::TakeDamage(float DamageAmount, FDamageEvent const& DamageEvent, AController* EventInstigator,
                                      AActor* DamageCauser)
{
...
void ADodgeBallCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = CastChecked<UEnhancedInputComponent>(PlayerInputComponent)) {
		...
		//Shooting
		EnhancedInputComponent->BindAction(ShootAction, ETriggerEvent::Triggered, this, &ADodgeBallCharacter::Throw);
